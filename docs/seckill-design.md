...
2. **网关层令牌桶限流**：秒杀请求到达 `octopus-gateway` 后，利用网关的全局过滤器结合 **Bucket4j** (底层依赖 Redis) 获取令牌。获取失败立即返回 HTTP 429 或定制的“活动火爆，请重试”响应。
...
**Plaza 实现方案（Gateway + Bucket4j + Redis）**：
- 由于系统引入了 `octopus-gateway`，全局限流的最佳实践是将其放在**网关层**。网关作为所有微服务请求的统一入口，能最有效地阻挡无效流量，避免它们消耗下游微服务（如 Web 容器线程池）的资源。
- 方案选择：在 `octopus-gateway` 项目中，基于 **Bucket4j** (底层结合 Redis) 实现分布式令牌桶限流。
- Bucket4j 提供了高效的并发控制，并在 Redis 中记录两个关键属性：上次补充令牌的时间和当前桶中剩余令牌数。
- 每次请求经过网关时，网关通过 Bucket4j 的 API 根据当前时间与上次补充时间的差值计算出期间应补充的令牌数，更新剩余令牌，然后尝试扣减。扣减成功则路由到 `plaza-order` 或秒杀服务，失败则直接在网关层返回 HTTP 429。
...
### 5.2 抢购阶段
1. **网关限流拦截**：秒杀请求到达 `octopus-gateway`，网关利用 **Bucket4j** (结合 Redis) 获取令牌。获取成功则放行至业务微服务，失败则直接返回“活动火爆”。
